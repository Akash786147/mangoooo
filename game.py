...
class Game:
    ...
    def __init__(self):
        ...
        # Initialize font for timer and UI text
        pygame.font.init()
    ...
    def draw_timer(self, elapsed_time):
        ...
        # Create text surface with golden/yellow color
        text_surface = self.font.render(time_text, True, (255, 215, 0))
        
        # Position at top center
        text_x = (self.display.get_width() - text_surface.get_width()) / 2
        text_y = 10
        ...
        pygame.draw.rect(self.display, (100, 80, 50), bg_rect, 2)
        
        self.display.blit(text_surface, (text_x, text_y))
    
    def draw_collectibles(self, collectibles_manager):
        """
        Draw all uncollected diamonds.
        
        Args:
            collectibles_manager: CollectiblesManager - Manager for all collectibles
        """
        for diamond in collectibles_manager.get_uncollected_diamonds():
            self.display.blit(diamond.get_image(), diamond.get_location())
    
    def draw_diamond_counter(self, collected, total):
        """
        Draw diamond collection counter.
        
        Args:
            collected: int - Number of diamonds collected
            total: int - Total number of diamonds
        """
        counter_text = f"Diamonds: {collected}/{total}"
        text_surface = self.small_font.render(counter_text, True, (255, 255, 255))
        ...
        # Draw background
        bg_rect = pygame.Rect(text_x - 5, text_y - 3, 
                             text_surface.get_width() + 10, 
                             text_surface.get_height() + 6)
        pygame.draw.rect(self.display, (0, 0, 0), bg_rect)
        pygame.draw.rect(self.display, (100, 100, 100), bg_rect, 2)
